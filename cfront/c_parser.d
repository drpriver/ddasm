...
struct CParser {
        ...
        do {
            ...
            if(infer_type){
                ...
                if(match(CTokenType.EQUAL)){
                    ...
                    // Infer array size from initializer for int arr[] = {...}
                    if(var_type.is_array() && var_type.array_size == 0){
                        if(CInitList* init_list = initializer.as_init_list()){
                            // Count elements (for nested structs, each init element = one array element)
                            size_t total = init_list.elements.length;
                            var_type.array_size = total;
                        } else if(CLiteral* lit = initializer.as_literal()){
                            // String literal for char[]
                            if(var_type.pointed_to && var_type.pointed_to.kind == CTypeKind.CHAR){
                                if(lit.value.type == CTokenType.STRING){
                                    size_t char_count = 0;
                                    str s = lit.value.lexeme;
                                    for(size_t si = 0; si < s.length; si++){
                                        if(s[si] == '\\' && si + 1 < s.length) si++;
                                        char_count++;
                                    }
                                    char_count++;  // null terminator
                                    var_type.array_size = char_count;
                                }
                            }
                        }
                    }
                    ...
                    if(initializer.kind != CExprKind.INIT_LIST){
                        ...
                        // Normalize init list to match target type structure
                        // (handles C's flat initializer "flood fill" semantics)
                        initializer = normalize_init_list(initializer.as_init_list(), var_type);

                        // For scalar types, unwrap single-element init lists: int x = {42} -> int x = 42
                        if(var_type !is null && !var_type.is_struct_or_union() && !var_type.is_array()){
                            initializer = unwrap_scalar_init_list(initializer, var_type, name);
                        }
                    }
                }
            }
    ...
    // (6.7.9) Normalize initializer list to match target type structure.
    // C allows flat initializers like {1, 2, 3, 4} to initialize nested aggregates.
    // This function restructures such flat lists into properly nested form.
    CExpr* normalize_init_list(CInitList* init_list, CType* target_type){
        if(init_list is null || target_type is null) return &init_list.expr;

        // Scalars don't need restructuring
        if(!target_type.is_struct_or_union() && !target_type.is_array()){
            return &init_list.expr;
        }

        // If all elements have designators, no restructuring needed
        bool all_designated = true;
        foreach(ref elem; init_list.elements){
            if(elem.designators.length == 0){
                all_designated = false;
                break;
            }
        }
        if(all_designated && init_list.elements.length > 0){
            return &init_list.expr;
        }

        // Restructure: consume elements from flat list to build nested structure
        size_t cursor = 0;
        auto new_elements = make_barray!CInitElement(allocator);

        if(target_type.is_struct_or_union()){
            auto fields = target_type.fields;
            foreach(ref field; fields){
                if(cursor >= init_list.elements.length) break;

                auto elem = init_list.elements[cursor];

                // Handle designators (skip to designated field)
                if(elem.designators.length > 0){
                    // Designators are handled separately, just pass through
                    new_elements ~= elem;
                    cursor++;
                    continue;
                }

                // Check if element is already a braced init list
                if(elem.value.as_init_list() !is null){
                    // Explicitly braced - normalize recursively and use as-is
                    CInitList* nested = elem.value.as_init_list();
                    CInitElement new_elem;
                    new_elem.value = normalize_init_list(nested, field.type);
                    // For scalar field types, unwrap the init list: struct S s = {{42}} -> {42}
                    if(field.type !is null && !field.type.is_struct_or_union() && !field.type.is_array()){
                        new_elem.value = unwrap_scalar_init_list(new_elem.value, field.type, init_list.expr.token);
                    }
                    new_elements ~= new_elem;
                    cursor++;
                } else if(field.type.is_struct_or_union() || field.type.is_array()){
                    // Field is aggregate - check if element is already a matching aggregate expression
                    // (e.g., a struct variable being used to initialize a struct field)
                    bool elem_is_matching_aggregate = false;
                    if(elem.value.type !is null){
                        if(field.type.is_struct_or_union() && elem.value.type.is_struct_or_union()){
                            // Element is a struct expression - pass through as-is
                            elem_is_matching_aggregate = true;
                        } else if(field.type.is_array() && elem.value.type.is_array()){
                            // Element is an array expression - pass through as-is
                            elem_is_matching_aggregate = true;
                        }
                    }

                    if(elem_is_matching_aggregate){
                        // Element already matches the aggregate type - pass through
                        new_elements ~= elem;
                        cursor++;
                    } else {
                        // Element is scalar - consume multiple elements to fill the aggregate
                        size_t n_scalars = count_scalar_subobjects(field.type);
                        size_t available = init_list.elements.length - cursor;
                        size_t to_consume = n_scalars < available ? n_scalars : available;

                        // Build sub-init-list from consumed elements
                        auto sub_elements = make_barray!CInitElement(allocator);
                        for(size_t j = 0; j < to_consume; j++){
                            sub_elements ~= init_list.elements[cursor + j];
                        }
                        CExpr* sub_list = CInitList.make(allocator, sub_elements[], init_list.expr.token);
                        // Recursively normalize the sub-list
                        CInitElement new_elem;
                        new_elem.value = normalize_init_list(sub_list.as_init_list(), field.type);
                        new_elements ~= new_elem;
                        cursor += to_consume;
                    }
                } else {
                    // Scalar field, scalar element - pass through
                    new_elements ~= elem;
                    cursor++;
                }
            }
            ...
            // If array size is 0 (to be deduced from initializer), don't restructure
            if(arr_size == 0){
                return &init_list.expr;
            }

            for(size_t i = 0; i < arr_size && cursor < init_list.elements.length; i++){
                auto elem = init_list.elements[cursor];

                if(elem.designators.length > 0){
                    // Designator - pass through
                    new_elements ~= elem;
                    cursor++;
                    continue;
                }
                ...
                if(elem.value.as_init_list() !is null){
                    ...
                    CInitList* nested = elem.value.as_init_list();
                    CInitElement new_elem;
                    new_elem.value = normalize_init_list(nested, elem_type);
                    // For scalar element types, unwrap the init list: int arr[] = {{1}, {2}} -> {1, 2}
                    if(elem_type !is null && !elem_type.is_struct_or_union() && !elem_type.is_array()){
                        new_elem.value = unwrap_scalar_init_list(new_elem.value, elem_type, init_list.expr.token);
                    }
                    new_elements ~= new_elem;
                    cursor++;
                } else if(elem_type.is_struct_or_union() || elem_type.is_array()){
                    // Array element is aggregate - check if value is already a matching aggregate
                    bool elem_is_matching_aggregate = false;
                    if(elem.value.type !is null){
                        if(elem_type.is_struct_or_union() && elem.value.type.is_struct_or_union()){
                            elem_is_matching_aggregate = true;
                        } else if(elem_type.is_array() && elem.value.type.is_array()){
                            elem_is_matching_aggregate = true;
                        }
                    }
                    ...
                    if(elem_is_matching_aggregate){
                        ...
                        // Element is scalar - consume multiple elements
                        size_t n_scalars = count_scalar_subobjects(elem_type);
                        size_t available = init_list.elements.length - cursor;
                        size_t to_consume = n_scalars < available ? n_scalars : available;
                        ...
                        CExpr* sub_list = CInitList.make(allocator, sub_elements[], init_list.expr.token);
                        CInitElement new_elem;
                        new_elem.value = normalize_init_list(sub_list.as_init_list(), elem_type);
                        new_elements ~= new_elem;
                        cursor += to_consume;
                    }
                } else {
        ...
        return CInitList.make(allocator, new_elements[], init_list.expr.token);
    }

    // Count the number of scalar subobjects in an aggregate type (for init list restructuring)
    size_t count_scalar_subobjects(CType* t){
        if(t is null) return 1;
        if(t.is_struct_or_union()){
            size_t count = 0;
            foreach(ref field; t.fields){
                count += count_scalar_subobjects(field.type);
            }
            return count > 0 ? count : 1;
        } else if(t.is_array()){
            return t.array_size * count_scalar_subobjects(t.pointed_to);
        } else {
            return 1;  // Scalar
        }
    }

    // Unwrap braced init list for scalar types: int x = {42} -> int x = 42
    // Handles nested braces: int x = {{{{42}}}} -> int x = 42
    CExpr* unwrap_scalar_init_list(CExpr* expr, CType* target_type, CToken tok){
        while(expr.kind == CExprKind.INIT_LIST){
            CInitList* init_list = expr.as_init_list();
            if(init_list.elements.length == 0){
                // Empty init list for scalar = zero
                return CLiteral.make_int(allocator, 0, tok);
            }
            if(init_list.elements.length != 1){
                error(tok, "Scalar initializer cannot have more than one element");
                return expr;
            }
            // Unwrap the single element
            expr = init_list.elements[0].value;
        }
        // Apply implicit cast to the unwrapped value
        return implicit_cast(expr, target_type, tok);
    }
